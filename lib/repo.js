...
(function() {
  ...
  App.Repo = (function() {
    ...
    Repo.prototype.trendData = function() {
      var result;
      result = App.Metrics.groupByWeek(this.rawdata.issues, 'updatedAt').map(function(weekOfIssues) {
        return [format(_.first(weekOfIssues).updatedAt), Number(sprintf('%.1f', App.Metrics.effectivenessForIssues(weekOfIssues)))];
      });
      console.log(result);
      return result;
    };

    format = function(aDate) {
      var day, month;

...
(function() {
  ...
  App.UI = (function() {
    ...
    UI.refreshLog = function(repos) {
      var tbody;
      tbody = "<tbody>" + (this.logRows(repos)) + "</tbody>";
      $('table#log tbody').replaceWith(tbody);
      return $('#avg-effectiveness').text(this.average(repos, (function(r) {
    ...
    UI.logRows = function(repos) {
      var rows;
      return rows = repos.reduce((function(_this) {
        return function(acc, r) {
          return acc + _this.tr(_this.td(_this.linkedRepoName(r)), _this.td(sprintf('%.1f', r.effectiveness())), _this.td('(tbd)', ''));
        };
      })(this), '');
    };

    UI.linkedRepoName = function(repo) {
      return ("<a href=" + (repo.url()) + " target=_blank>") + (repo.name + "<img class=external-link-icon src=external_link_icon.png></a>");
    };
    ...
    UI.td = function(text, htmlClass) {
      if (htmlClass == null) {
        htmlClass = 'data';
      }
      return "<td class='" + htmlClass + "'>" + text + "</td>";
    };
    ...
    UI.hideResultsDisplay = function() {
      return $('.results-display, #trend-chart').hide();
    };

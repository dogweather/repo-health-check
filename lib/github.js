...
(function() {
  App.Github = (function() {
    ...
    Github.parseRepoInput = function(text) {
      var matches;
      if (this.isRepoSpec(text)) {
        return text.split('/');
    ...
    Github.isUrl = function(text) {
      return /^https:\/\/github.com\/[^\/]+\/[^\/]+/.test(text);
    };

    Github.isRepoSpec = function(text) {
      return /^[^\/]+\/[^\/]+$/.test(text);
    };
    ...
    Github.percentComplete = function(apiResult) {
      return parseInt(this.currentPage(apiResult) / this.pageCount(apiResult) * 100);
    };
